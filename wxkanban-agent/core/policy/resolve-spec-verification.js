...
exports.resolveSpecVerification = resolveSpecVerification;
...
async function resolveSpecVerification(db, projectId) {
    // 1. Find the active scope. If 0 or >1 active scopes, treat as a
    //    precondition violation per spec 030 FR-004 corner-case rules.
    const scopeResult = await db.query(`SELECT id, status
       FROM projectspecifications
      WHERE projectid = $1
        AND status = ANY($2::text[])`, [projectId, ACTIVE_SCOPE_STATUSES]);
    ...
    const [tasksResult, docsResult] = await Promise.all([
        db.query(`SELECT COUNT(*)::text AS c FROM projecttasks WHERE specid = $1`, [activeScope.id]),
        db.query(`SELECT COUNT(*)::text AS c FROM projectdocuments WHERE specid = $1`, [activeScope.id]),
    ]);
...
//# sourceMappingURL=resolve-spec-verification.js.map