...
// cwConversionScope orchestrator command handler (the Clarion counterpart to
// wxconversionscope.ts).
//
// "Workflow engine, not AI client": runs NO AI. It installs the
// cwConversionScope skill DIRECTORY into the consumer's `.claude/` and verifies
// the conversion artifacts exist (`pre-convert/`); the scopes themselves land in
// `specs/Project-Scope/`, owned by the existing buildscope/createSpecs pipeline,
// so nothing is scaffolded here. The actual scope generation is driven by the
// developer's editor AI following the skill (methodology fetched via
// project.get_command_prompt { command: "cwconversionscope" }).

export interface CwConversionScopeOptions {
...
export interface CwConversionScopeAction {
  path: string;
  action: "created-dir" | "exists" | "created" | "skipped";
}

export interface CwConversionScopeResult {
  exitCode: 0 | 1 | 2;
  output: string;
  actions: CwConversionScopeAction[];
}

const SKILL_DIRNAME = "cwConversionScope";

export function handleCwConversionScopeCommand(
  opts: CwConversionScopeOptions = {},
): CwConversionScopeResult {
  ...
  if (!existsSync(join(skillSrc, "SKILL.md"))) {
    return {
      exitCode: 1,
      output: `ERROR: cwConversionScope skill template not found at ${skillSrc}\n`,
      actions: [],
    };
  }
  ...
  if (!existsSync(preConvert)) {
    return {
      ...
      output:
        ...
        "Run `wxkanban-agent cwconversion` and the conversion stages first to convert the\n" +
        "Clarion source, then run cwconversionscope to generate scopes from the result.\n",
      actions: [],
    };
  }

  const actions: CwConversionScopeAction[] = [];
  ...
  const output = [
    "cwConversionScope ready.",
    "",
    `  pre-convert/   found (conversion artifacts present)`,
    `  ${installedNote}`,
    "",
    "Next step (run in your editor AI):",
    "  Invoke /cwConversionScope to begin scope generation (resumable).",
