...
// vbConversionScope orchestrator command handler (the VB6 counterpart to
// cwconversionscope.ts). Runs NO AI: installs the vbConversionScope skill and
// verifies the conversion artifacts exist (`pre-convert/`); scopes land in
// `specs/Project-Scope/`, owned by the buildscope/createSpecs pipeline. The
// actual scope generation is driven by the editor AI following the skill
// (methodology via project.get_command_prompt {command: "vbconversionscope"}).

export interface VbConversionScopeOptions {
  projectRoot?: string;
  templatesDir?: string;
  force?: boolean;
}

export interface VbConversionScopeAction {
  path: string;
  action: "created-dir" | "exists" | "created" | "skipped";
}

export interface VbConversionScopeResult {
  exitCode: 0 | 1 | 2;
  output: string;
  actions: VbConversionScopeAction[];
}

const SKILL_DIRNAME = "vbConversionScope";

export function handleVbConversionScopeCommand(
  opts: VbConversionScopeOptions = {},
): VbConversionScopeResult {
  ...
  if (!existsSync(join(skillSrc, "SKILL.md"))) {
    return {
      exitCode: 1,
      output: `ERROR: vbConversionScope skill template not found at ${skillSrc}\n`,
      actions: [],
    };
  }
  ...
  if (!existsSync(preConvert)) {
    return {
      ...
      output:
        ...
        "Run `wxkanban-agent vbconversion` and the conversion stages first to convert the\n" +
        "VB6 source, then run vbconversionscope to generate scopes from the result.\n",
      actions: [],
    };
  }

  const actions: VbConversionScopeAction[] = [];
  ...
  const output = [
    "vbConversionScope ready.",
    "",
    `  pre-convert/   found (conversion artifacts present)`,
    `  ${installedNote}`,
    "",
    "Next step (run in your editor AI):",
    "  Invoke /vbConversionScope to begin scope generation (resumable).",
