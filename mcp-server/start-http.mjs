...
// BUG-11: resolve dist/index-http.js relative to this script, not cwd, so
// `node mcp-server/start-http.mjs` works from any directory (previously
// only ran when invoked from inside mcp-server/).
...
const proc = spawn('node', [join(here, 'dist', 'index-http.js')], {
  stdio: 'inherit',
  cwd: here,
  env: {
    ...process.env,
